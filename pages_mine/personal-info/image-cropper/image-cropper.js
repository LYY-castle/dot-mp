Component({
	...
	methods: {
		...
		setTransform(transform) {
			...
			if (!this.data.disable_rotate) {
				this.setData({
					angle: transform.angle
						? this.data.angle + transform.angle
						: this.data.angle
				})
			}
			...
			this.data._img_top = transform.y
				? this.data._img_top + transform.y
				: this.data._img_top
			this.data._img_left = transform.x
				? this.data._img_left + transform.x
				: this.data._img_left
			//图像边缘检测,防止截取到空白
		...
		_canvasDetectionPosition() {
			if (this.data.canvas_top == null && this.data.canvas_left == null) {
				...
				this.data.canvas_top != null &&
				this.data.canvas_left != null
			) {
				if (
		...
		_move(event) {
			...
			if (event.touches.length == 1) {
				...
				if (!this.data.disable_rotate) {
					let first_atan =
						(180 / Math.PI) *
						Math.atan2(_touch_img_relative[0].y, _touch_img_relative[0].x)
					let first_atan_old =
						(180 / Math.PI) *
						Math.atan2(
							this.data._touch_img_relative[0].y,
							this.data._touch_img_relative[0].x
						)
					let second_atan =
						(180 / Math.PI) *
						Math.atan2(_touch_img_relative[1].y, _touch_img_relative[1].x)
					let second_atan_old =
						(180 / Math.PI) *
						Math.atan2(
							this.data._touch_img_relative[1].y,
							this.data._touch_img_relative[1].x
						)
					//当前旋转的角度
		...
		_draw(callback) {
			...
			let draw = () => {
				//图片实际大小
				let img_width =
					this.data.img_width * this.data.scale * this.data.export_scale
				let img_height =
					this.data.img_height * this.data.scale * this.data.export_scale
				//canvas和图片的相对距离
		...
		_cutTouchMove(e) {
			if (this.data._flag_cut_touch && this.data.MOVE_THROTTLE_FLAG) {
				if (
					...
					(this.data.disable_width || this.data.disable_height)
				)
					return
				//节流
				this.data.MOVE_THROTTLE_FLAG = false
				...
				height =
					...
					(this.data.CUT_START.corner > 1 && this.data.CUT_START.corner < 4
						? 1
						: -1) *
				...
				switch (this.data.CUT_START.corner) {
					case 1:
						width =
							this.data.CUT_START.width +
							this.data.CUT_START.x -
							e.touches[0].clientX
						...
						cut_left =
							this.data.CUT_START.cut_left - (width - this.data.CUT_START.width)
						break
					case 2:
						width =
							this.data.CUT_START.width +
						...
						if (!size_inspect()) return
						cut_top =
							this.data.CUT_START.cut_top -
							(height - this.data.CUT_START.height)
						cut_left =
							this.data.CUT_START.cut_left - (width - this.data.CUT_START.width)
						break
					case 3:
